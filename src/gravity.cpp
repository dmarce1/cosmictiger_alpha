...
void tree::cpu_pp_direct(kick_params_type *params_ptr) {
	...
	for (int pi = 0; pi < NPART_TYPES; pi++) {
		...
		for (int i = 0; i < nparts; i++) {
			const auto& particles = params.part_sets->sets[pi];
			for (int dim = 0; dim < NDIM; dim++) {
				X[dim] = particles->pos(dim, i + parts[pi].first).raw();
			}
			if (particles->rung(i + parts[pi].first) >= params.rung || params.full_eval) {
				...
				for (int dim = 0; dim < NDIM; dim++) {
					F[dim][i + set_offset] -= f[dim].sum();
				}
				Phi[i + set_offset] += phi.sum();
			}
		}
	}
	if (params.full_eval) {
	...
	for (int pi = 0; pi < NPART_TYPES; pi++) {
		...
		for (int i = 0; i < nparts; i++) {
			...
			if (particles->rung(i + parts[pi].first) >= params.rung || params.full_eval) {
				...
				Phi[i + offset] += Lacc[0].sum();                                                               // * DSCALE;
				for (int dim = 0; dim < NDIM; dim++) {
					F[dim][i + offset] -= Lacc[1 + dim].sum();                                        // * (DSCALE * DSCALE);
				}
			}
		}
	}
