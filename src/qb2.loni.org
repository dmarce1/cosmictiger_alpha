				...
				for( int k = lb[2]; k < ub[2]; k += 2 ) {
					...
					for (int i0 = 0; i0 < 2; i0++) {
						for (int j0 = 0; j0 < 2; j0++) {
							for (int k0 = 0; k0 < 2; k0++) {
								u0 += U[field][hindex(i + i0, j + j0, k + k0)];
							}
						}
					}
					u0 /= 8.0;

					slp[XDIM] =  (U[field][iii + H_DNX]                 - U[field][iii]);
					slp[XDIM] += (U[field][iii + H_DNX + H_DNY]         - U[field][iii + H_DNY]);
					slp[XDIM] += (U[field][iii + H_DNX + H_DNZ]         - U[field][iii + H_DNZ]);
					slp[XDIM] += (U[field][iii + H_DNX + H_DNY + H_DNZ] - U[field][iii + H_DNY + H_DNZ]);

					slp[YDIM] =  (U[field][iii + H_DNY]                 - U[field][iii]);
					slp[YDIM] += (U[field][iii + H_DNY + H_DNX]         - U[field][iii + H_DNX]);
					slp[YDIM] += (U[field][iii + H_DNY + H_DNZ]         - U[field][iii + H_DNZ]);
					slp[YDIM] += (U[field][iii + H_DNY + H_DNX + H_DNZ] - U[field][iii + H_DNX + H_DNZ]);

					slp[ZDIM] =  (U[field][iii + H_DNZ]                 - U[field][iii]);
					slp[ZDIM] += (U[field][iii + H_DNZ + H_DNY]         - U[field][iii + H_DNY]);
					slp[ZDIM] += (U[field][iii + H_DNZ + H_DNX]         - U[field][iii + H_DNX]);
					slp[ZDIM] += (U[field][iii + H_DNZ + H_DNY + H_DNX] - U[field][iii + H_DNY + H_DNX]);
					...
					for( int side = 0; side < 2; side++) {
						for( int dim = 0; dim < NDIM; dim++) {
							...
							const int dima = dim == XDIM ? YDIM : XDIM;
							const int dimb = dim == ZDIM ? YDIM : ZDIM;
							const int sgn = 2 * side - 1;
							const int str0 = D[dim];
							const int stra = D[dima];
							const int strb = D[dimb];

							const auto& Uf = U[field];
					...
					//				printf( "%e ", slp[dim]);
								...
								{
									const auto& u1 = Uf[iii + sgn*str0];
									const auto& u2 = Uf[iii + sgn*str0 + stra];
									const auto& u3 = Uf[iii + sgn*str0 + strb];
									const auto& u4 = Uf[iii + sgn*str0 + stra + strb];
							//		slp[dim] = minmod( slp[dim], sgn*((u1 + u2 + u3 + u4)*0.25 - u0)/1.5);
							//		slp[dim] = minmod( slp[dim], 2.0*sgn*(u1-u0));
							//		slp[dim] = minmod( slp[dim], 2.0*sgn*(u2-u0));
							//		slp[dim] = minmod( slp[dim], 2.0*sgn*(u3-u0));
							//		slp[dim] = minmod( slp[dim], 2.0*sgn*(u4-u0));
								}
								if( f) {
				//					printf( "%e\n", slp[dim]);
								}

						//	slp[dima] = minmod( slp[dima], Uf[iii + sgn*str0 + stra       ] - Uf[iii + sgn*str0       ] );
//							slp[dima] = minmod( slp[dima], Uf[iii + sgn*str0 + stra + strb] - Uf[iii + sgn*str0 + strb] );

	//						slp[dimb] = minmod( slp[dimb], Uf[iii + sgn*str0 + strb       ] - Uf[iii + sgn*str0       ] );
		//					slp[dimb] = minmod( slp[dimb], Uf[iii + sgn*str0 + strb + stra] - Uf[iii + sgn*str0 + stra] );
							}
						}
					}
					...
					for (int i0 = 0; i0 < 2; i0++) {
						...
						for (int j0 = 0; j0 < 2; j0++) {
							...
							for (int k0 = 0; k0 < 2; k0++) {
								int ksgn = 2 * k0 - 1;
								auto& this_U = U[field][hindex(i + i0, j + j0,
										k + k0)];
								this_U = u0;
								this_U += 0.5 * isgn * slp[XDIM];
								this_U += 0.5 * jsgn * slp[YDIM];
								this_U += 0.5 * ksgn * slp[ZDIM];
							}
						}
					}
				}
...
//	if (opts().hydro) {
	for (auto l : str_to_index_hydro) {
		...
		for (int i = 0; i < INX; i++) {
			for (int j = 0; j < INX; j++) {
				for (int k = 0; k < INX; k++) {
					const int iii = hindex(k + 2*H_BW , j + H_BW, i + H_BW);
