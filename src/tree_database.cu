...
void tree_data_initialize_kick() {
	...
	cpu_tree_data_.ntrees = 5 * global().opts.nparts / global().opts.bucket_size;
	if (global().opts.sph) {
...
//	CUDA_CHECK(cudaMemAdvise(&cpu_tree_data_, sizeof(cpu_tree_data_), cudaMemAdviseSetReadMostly, 0));
	...
	CUDA_MALLOC(cpu_tree_data_.all_local, cpu_tree_data_.ntrees);
	if (global().opts.sph) {
		CUDA_MALLOC(cpu_tree_data_.ranges, cpu_tree_data_.ntrees);
		CUDA_MALLOC(cpu_tree_data_.sph_ranges, cpu_tree_data_.ntrees);
	}
	tree_data_clear();
...
void tree_data_initialize_groups() {
	cpu_tree_data_.chunk_size = 1;
	cpu_tree_data_.ntrees = 4 * global().opts.nparts / GROUP_BUCKET_SIZE;
	cpu_tree_data_.ntrees = std::max(cpu_tree_data_.ntrees, min_trees);
	const int target_chunk_size = cpu_tree_data_.ntrees / (16 * OVERSUBSCRIPTION * hardware_concurrency());
	while (cpu_tree_data_.chunk_size < target_chunk_size) {
		cpu_tree_data_.chunk_size *= 2;
