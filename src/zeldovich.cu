...
__global__
void zeldovich(cmplx* phi, const cmplx* basis, const cmplx* rands, const interp_functor<float>* Pptr,
		float box_size, int N, int dim, zeldovich_t type) {
			...
			if (!((l == 0) && (j == 0) && (i >= N / 2)) && !((l == 0) && (j >= N / 2))) {
				if (i2 > 0 && i2 < N * N / 4) {
					...
					case VELOCITY:
						...
						break;
					case DISPLACEMENT:
						phi[index0] = -cmplx(0, 1) * (rands[index0] * sqrtf(P(k))) * K[dim]
								/ (k * k)* powf(box_size, -1.5);
						break;
					case DENSITY:
