...
std::vector<size_t> particle_set::local_sort(size_t start, size_t stop, int64_t depth, morton_t key_min,
   ...
   timer tm;
   tm.start();
   auto bounds = cuda_keygen(*this, start, stop, depth, key_min, key_max);
   std::vector < size_t > &end = bounds.second;
   std::vector < size_t > &begin = bounds.first;
   //  printf("-----------\n");
...
//   printf("-----------\n");
   ...
   tm.stop();
   printf("Final Sort took %e s, %i sorted.\n", tm.read(), sorted);
