...
void particle_set::prepare_kick(size_t begin, size_t end, cudaStream_t stream) {
	return;
...
//		CUDA_CHECK(cudaMemPrefetchAsync(xptr_[dim] + begin, size * sizeof(fixed32), 0, stream));
	...
	CUDA_CHECK(cudaMemAdvise(rptr_ + begin, size * sizeof(int8_t), cudaMemAdviseSetPreferredLocation, 0));
//	CUDA_CHECK(cudaMemPrefetchAsync(rptr_ + begin, size * sizeof(int8_t), 0, stream));
}
