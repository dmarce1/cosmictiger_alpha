...
CUDA_KERNEL morton_keygen(uint64_t *__restrict__ keys, fixed32 *__restrict__ xptr, fixed32 *__restrict__ yptr,
		fixed32 *__restrict__ zptr, size_t nele, size_t depth) {
...
std::vector<size_t> cuda_keygen(particle_set &set, size_t start, size_t stop, int depth, morton_t key_start,
		morton_t key_stop, cudaStream_t stream) {
	...
	/***********************************************************************************/
	/**/morton_keygen<<<nkeyblocks, KEY_BLOCK_SIZE, 0, stream>>>(keys,x,y,z,stop-start, depth);/**/
	/**/
