#include <cosmictiger/defs.hpp>
#include <cosmictiger/hpx.hpp>
#include <cosmictiger/particle.hpp>
#include <cosmictiger/map.hpp>
#include <cosmictiger/global.hpp>
#include <cosmictiger/cosmos.hpp>


struct drift_params_t {
	particle_set parts;
	double dt;
	double a;
	double* ekin;
	double* momx;
	double* momy;
	double* momz;
	double tau;
	double tau_max;
	int rc;
	int bid;

};

void cpu_drift_kernel(void* params_ptr) {
	drift_params_t& params = *(drift_params_t*) params_ptr;
	particle_set parts = params.parts;
	const int gsz = 2 * hpx::threads::hardware_concurrency();
	double dt = params.dt;
	double a = params.a;
	double myekin = 0.0;
	double mymomx = 0.0;
	double mymomy = 0.0;
	double mymomz = 0.0;
	double tau = params.tau;
	int bid = params.bid;
	double tau_max = params.tau_max;
	const bool map = global().opts.map_size > 0;
	const size_t nparts = parts.size();
	const size_t start = bid * nparts / gsz;
	const size_t stop = (bid + 1) * nparts / gsz;
	const float ainv = 1.0 / a;
	const float dteff = dt * ainv;
	int myrc = 0;
	myekin = 0;
	mymomx = 0;
	mymomy = 0;
	mymomz = 0;
	array<double,NDIM> x0, x1;
	printf( "%i %i\n", start, stop);
	for (size_t i = start; i < stop; i++) {
		double x = parts.pos(0, i).to_double();
		double y = parts.pos(1, i).to_double();
		double z = parts.pos(2, i).to_double();
		x0[0] = x;
		x0[1] = y;
		x0[2] = z;
		const float vx = parts.vel(i).p.x;
		const float vy = parts.vel(i).p.y;
		const float vz = parts.vel(i).p.z;
		const float ux = vx * ainv;
		const float uy = vy * ainv;
		const float uz = vz * ainv;
		myekin += 0.5 * (ux * ux + uy * uy + uz * uz);
		mymomx += ux;
		mymomy += uy;
		mymomz += uz;
		x += vx * dteff;
		y += vy * dteff;
		z += vz * dteff;
		x1[0] = x;
		x1[1] = y;
		x1[2] = z;
		if( map ) {
			myrc +=map_add_part(x0, x1, tau, dt, tau_max);
		}
		while (x >= 1.0) {
			x -= 1.0;
		}
		while (y >= 1.0) {
			y -= 1.0;
		}
		while (z >= 1.0) {
			z -= 1.0;
		}
		while (x < 0.0) {
			x += 1.0;
		}
		while (y < 0.0) {
			y += 1.0;
		}
		while (z < 0.0) {
			z += 1.0;
		}
		parts.pos(0, i) = x;
		parts.pos(1, i) = y;
		parts.pos(2, i) = z;
	}
	*params.ekin = myekin;
	*params.momx = mymomx;
	*params.momy = mymomy;
	*params.momz =  mymomz;
	params.rc = myrc;
}


int drift_particles(particle_set parts, double dt, double a0, double* ekin, double* momx, double* momy, double* momz,
		double tau, double tau_max) {
	static mutex_type mtx;
	std::vector<hpx::future<void>> futs;
	static std::atomic<int> rc(0);
	const auto ddt = cosmos_drift_dtau(a0,dt);
	const auto a = 1.0 / ddt;
	const int gsz = 2 * hpx::threads::hardware_concurrency();
	*ekin = *momx = *momy = *momz = 0.0;
	rc = 0;
	for (int bid = 0; bid < gsz; bid++) {
		auto func = [bid, gsz, &parts, dt,a,ekin, momx, momy, momz, tau, tau_max]() {
			auto params_ptr = std::make_shared<drift_params_t>();
			auto& params = *params_ptr;
			params.a = a;
			params.dt = dt;
			params.ekin = ekin;
			params.momx = momx;
			params.momy = momy;
			params.momz = momz;
			params.parts = parts;
			params.bid = bid;
			params.tau = tau;
			params.tau_max = tau_max;
			cpu_drift_kernel(&params);
			std::lock_guard<mutex_type> lock(mtx);
			*ekin += *params.ekin;
			*momx += *params.momx;
			*momy += *params.momy;
			*momz += *params.momz;
			rc += params.rc;
		};
		futs.push_back(hpx::async(func));
	}
	hpx::wait_all(futs.begin(), futs.end());
	return int(rc);
}

